...
class YoutubeProcessor:
    ...
    def find_key_concepts(self, documents: list, sample_size: int = 0, verbose= False):
        ...
        if sample_size == 0:
            ...
            if verbose:
                logging.info(f"No sample size specified. Setting number of documents per sample as 5.\n Sample size: {sample_size}")

        num_docs_per_group= len(documents) // sample_size + (len(documents) % sample_size > 0)

        if num_docs_per_group >=10:
            raise ValueError("Group size is too large. Increase the sample_size parameter to reduce the number of documents per group.")
        elif num_docs_per_group > 5:
            logging.warn("Each group has more than 5 documents and output quality likely to be degraded. consider increasing the sample size.")
        ...
        for group in tqdm(groups):
            ...
            prompt= PromptTemplate(
                template="""
                Find and define key concepts or terms found in text:
                {text}
